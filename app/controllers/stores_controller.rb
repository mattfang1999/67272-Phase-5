...
class StoresController < ApplicationController
  ...
  def index
    ...
    @active_stores = Store.active.alphabetical.paginate(page: params[:employee_page]).per_page(10)
  ...
  def show
    ...
    @current_employees = @store.assignments.current.map{|a| a.employee}.sort_by{|e| e.name}.paginate(page: params[:page], :per_page => 10)


    @upcoming_shifts = Shift.upcoming.for_next_days(7).for_store(@store).chronological.by_employee.paginate(page: params[:page]).per_page(5)
  end
